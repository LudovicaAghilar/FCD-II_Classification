...
def train_model(num_epochs, batch_size, lr, patience, optimizer, experiment_name='default_exp'):
    ...
    # Crea una trasformazione di rotazione casuale
    rand_rotate = tio.RandomAffine(
        scales=1,  # nessun ridimensionamento
        degrees=15,  # rotazione casuale fino a ±10 gradi su ciascun asse
        translation=0  # nessuna traslazione
    )
    dataset = NiftiDataset(root_dir=root_dir, label_mapping=label_mapping)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset, all_labels)):
        ...
        train_dataset = NiftiDataset(root_dir=root_dir, label_mapping=label_mapping, transform=rand_rotate)
        val_dataset = NiftiDataset(root_dir=root_dir, label_mapping=label_mapping, transform=None)
        test_dataset = NiftiDataset(root_dir=root_dir, label_mapping=label_mapping, transform=None)
        ...
        ## ATTEMPT PARAMETERS ##
        # Separiamo i parametri in due gruppi:
        """ base_parameters = []
        new_parameters = []

        for name, param in model.named_parameters():
            if param.requires_grad:
                if name.startswith("fc") or name.startswith("layer4") or name.startswith("avgpool"):
                    new_parameters.append(param)  # Parametri nuovi: aggiorniamo più rapidamente
                else:
                    base_parameters.append(param)  # Parametri base (es. layer1-3): aggiornamento più lento

        # Definizione dell’optimizer con LR differenziati
        optimizer = optim.Adam([
            {'params': base_parameters, 'lr': lr},
            {'params': new_parameters, 'lr': lr * 10}
        ]) """
